...
export const DashboardPost: FC<DashboardPostProps> = ({ post, uid, myPosts, deleteOnClick }) => {
  ...
  return (
    <div className='flex justify-between flex-col md:flex-row mt-4 px-4 py-6 rounded-lg bg-blue-100 shadow-lg'>
      {post.previewImage.url && (
        <div
          className='w-full md:w-32 mb-4 md:mb-0 md:h-20 rounded-lg shadow-lg overflow-hidden shrink-0 self-center inline-block hover:opacity-60 transition-opacity '
      ...
      <div className={`${post.previewImage.url && 'md:px-4 md:ml-4'}  flex-grow font-bold`}>
        <div
          className='text-lg text-gray-900 mb-4 cursor-pointer hover:opacity-60 transition-opacity  block'
          onClick={onClickNavigation}
        >
          {post.title}
        </div>
